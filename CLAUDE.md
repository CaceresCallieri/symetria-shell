...
**`readonly property` blocks ALL assignment** — `readonly` in QML means the property has ONE value source (its initializer) and forbids imperative assignment from *any* scope, including signal handlers in the same file — there is no "internal write" exception. A singleton property written imperatively by an internal `FileView`/`Process`/`Timer` handler must stay a plain writable `property`; marking it `readonly` makes the handler's assignment throw and silently no-op, freezing the value. `qmllint` can't catch this (it exits 255 on unresolved Quickshell imports). Symptom: `QuietMode.enabled` made `readonly` by a review → `FileView.onLoaded` write failed → Silent toggle frozen false. → `docs/qml-pitfalls.md`
