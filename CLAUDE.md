...
**A derived property reads stale inside a sibling's change handler** — sibling `readonly` bindings over the same source do not update atomically, so `onAChanged` can observe `B` still holding its pre-change value. A handler must re-derive its verdict from raw state, never from a sibling binding. And never write a binding's dependency synchronously from its own change handler: Qt aborts it as a binding loop and leaves the property **frozen at the stale value** (a corrupted property, not just a warning) — defer with `Qt.callLater`. Symptom: `SuspendTimer` suspended the machine the instant the toggle was armed, then read as "armed, 0:00" forever. → `docs/qml-pitfalls.md`
